...
//List of the queries
const qGetTeachers = 'select c.nom_curso,cc.nom_ciclo, pg.nom_programa, pl.id_planilla, pl.persona_id, pl.id_curso,  pl.id_ciclo, pl.t_horas, pl.t_unidad, pl.importe, p.persona_dni, p.persona_apaterno, p.persona_amaterno, p.persona_nombres from persona p inner join planilla pl on p.persona_id = pl.persona_id inner join curso c on c.id_curso=pl.id_curso inner join ciclo cc on cc.id_ciclo = pl.id_ciclo inner join programa pg on c.id_programa = pg.id_programa'
...
const qGetTeachersByDni = 'select p.persona_id, c.nom_curso, pro.nom_programa, p.persona_apaterno, p.persona_amaterno, p.persona_nombres, p.persona_dni, d.per_academico, pl.t_horas, pl.t_unidad, pl.importe from persona p INNER JOIN docente d ON d.nro_document=p.persona_dni inner join planilla pl on p.persona_id = pl.persona_id  inner join curso c on c.id_curso = pl.id_curso inner join programa pro on c.id_programa = pro.id_programa' + 
